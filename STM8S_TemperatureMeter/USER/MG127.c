...
uint8_t adv_data[31] = {
    ...
    //'L', 'o', 'v', 'e', '7', '-', 'D', 'M'  //数据"MG127-Demo"
    'L', 'o', 'v', 'e',' ','1',
  //数据
    //struoture 2
    0x11,                                           //长度
    BLE_GAP_AD_TYPE_128BIT_SERVICE_UUID_COMPLETE,   //数据类型 0x07
    0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x63, 0x00, 0x00, 0xaa, 0x1D, 0xe2, 0x06,0x00, //数据
...
void BLE_Init(uint8_t power)
{
  ...
  //  uint8_t ble_Addr[6];
    ...
    if (debug != 0x00) {
        UART1_SendString("Chip version=",sizeof("Chip version="));
        UART1_SendByte(status);
        UART1_SendString("\r\n",sizeof("\r\n"));
    }
#endif
    ...
    if (debug != 0x00) {
        UART1_SendString("BleAddr=",sizeof("BleAddr="));
        UART1_SendByte(ble_Addr[5]);
        UART1_SendByte(ble_Addr[4]);
        UART1_SendByte(ble_Addr[3]);
        UART1_SendByte(ble_Addr[2]);
        UART1_SendByte(ble_Addr[1]);
        UART1_SendByte(ble_Addr[0]);
        UART1_SendString("\r\n",sizeof("\r\n"));
    }
#endif
...
//    uint8_t loop = 0;
    ...
    while(1) {
           ...
           //     LED_RED_ON(); //debug
                ...
                if (debug) {
                    ...
                    for(loop = 0; loop < len_pdu; loop++) {
                        UART1_SendByte(rx_buf[loop]);
                        UART1_SendString(" ",sizeof(" "));
                    }
                    //Uart_Send_String("\r\n");
                }
#endif
     ...
     //           LED_RED_OFF();  //debug
        ...
        } else if(ble_tick == 0) { //robustness, in case no int
            BLE_Mode_Sleep();
        }
