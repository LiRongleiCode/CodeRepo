...
//串口透传服务BLE_NUS_EVT_TX_DATA事件数据结构体，该结构体用于当BLE_NUS_EVT_TX_DATA产生时将接收的数据信息传递给处理函数
typedef struct
{
    uint8_t const * p_data; //指向存放接收数据的缓存
    uint16_t        length; //接收的数据长度
} ble_uarts_evt_tx_data_t;
//记录对端设备是否使能了RX特征的通知
typedef struct
{
...
typedef struct
{
    ...
    ble_uarts_client_context_t   * p_link_ctx;
    union
    {
        ble_uarts_evt_rx_data_t rx_data; //BLE_NUS_EVT_RX_DATA事件数据
        ble_uarts_evt_tx_data_t tx_data; //BLE_NUS_EVT_TX_DATA事件数据
    } params;
} ble_uarts_evt_t;
...
//定义最大传输数据长度（字节数）
    ...
    #define BLE_UARTS_MAX_DATA_LEN (NRF_SDH_BLE_GATT_MAX_MTU_SIZE - OPCODE_LENGTH - HANDLE_LENGTH)  //244
#else
