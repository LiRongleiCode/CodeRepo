...
#define BLE_UARTS_MAX_RX_CHAR_LEN        BLE_UARTS_MAX_DATA_LEN //RX特征最大长度（字节数）  //244
#define BLE_UARTS_MAX_TX_CHAR_LEN        BLE_UARTS_MAX_DATA_LEN //TX特征最大长度（字节数）//244
...
static void on_connect(ble_uarts_t * p_uarts, ble_evt_t const * p_ble_evt)
{
    ...
    err_code = sd_ble_gatts_value_get(p_ble_evt->evt.gap_evt.conn_handle,
                                      p_uarts->tx_handles.cccd_handle,
                                      &gatts_val);                               //获取蓝牙从APP传过来的值。
    if ((err_code == NRF_SUCCESS)     &&
                           ...
                           uint16_t    conn_handle)
{
    ...
    //发送TX特征值通知
    return sd_ble_gatts_hvx(conn_handle, &hvx_params);   //数据通过蓝牙发送函数
}
